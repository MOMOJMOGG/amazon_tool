...
6. "Virtual environment preference?"
   - Create local `.venv` here
   - Use existing path: [provide path]
   - Conda env name: [name]
7. "Runtime version?" (e.g., Python 3.11.x / Node LTS)
8. **Dependency manager preference?** (pip/requirements, Poetry, PDM, pip-tools) — list only, do not install
9. **Test framework preference?** (pytest / unittest / vitest / jest / none)
```
...
# (AFTER creating tests/* files)
# STOP HERE for manual test run:
# "Tests are scaffolded but NOT executed. Please run tests manually now.
#  Paste any failing logs for me to fix, or type `continue` to proceed."
```
...
> **All shell commands below must be executed with an active virtual environment.**
> Claude Code must **prepend** activation to every command, e.g.:
> - macOS/Linux: `source .venv/bin/activate && <command>`
> - Windows PowerShell: `.venv\\Scripts\\Activate.ps1; <command>`
> - Conda: `conda activate <env-name> && <command>`
...
For Python projects Claude Code should:
1) **Infer** minimal dependencies per feature (e.g., `requests`, `pydantic`, `fastapi`, `uvicorn`, `pandas`, `pytest`, `ruff`, `black`, `mypy`… if relevant)
2) **Write**:
   - `requirements.txt` (runtime)
   - `requirements-dev.txt` (dev/test tools)
   - `docs/dependencies.md` (why each package is needed)
3) **Display** the list and **pause**:
   > "Dependencies listed in `requirements*.txt`. Please install them manually in your active venv.
   > Type **`continue`** when ready."
4) **Never** execute `pip install`/`poetry add`/`npm i` etc.
...
## 🚦 Error Report Format
When user pastes errors, Claude must request:
- Command executed
- Full error log
- OS + shell + Python/Node version
- Active venv path/name
